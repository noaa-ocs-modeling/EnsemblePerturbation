...
def parse_input(input_directory):
    ...
    print(ds_vortex)
    ds_vortex.to_hdf(input_directory.name + '.h5',
        key='vortex_perturbation_parameters',mode='w',format='table',data_columns=True,
    )

def parse_output(input_directory):
    # define the output file type and variable name interested name
    output_filetypes = {'maxele.63.nc': 'zeta_max',
    }
    ...
    output_data = parse_adcirc_outputs(directory=input_directory,
                     file_data_variables=output_filetypes.keys(),
    )
    ...
    for pdx, perturbation in enumerate(output_data):
        for vdx, variable in enumerate(output_data[perturbation]):
            ...
            if pdx == 0:
                ...
                lon = ds['x']
                lat = ds['y']
                subset = (depth < max_depth) & (lon > lon_min) & (lon < lon_max) & \
    ...
    print(ds_subset)
    ds_subset.to_hdf(input_directory.name + '.h5',
        key=output_filetypes[variable],mode='a',format='table',data_columns=True,
    )
