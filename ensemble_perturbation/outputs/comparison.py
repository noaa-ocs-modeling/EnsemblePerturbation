...
class ReferenceComparison:
    def __init__(self, input_directory: str, output_directory: str,
        ...
        self.geodetic = Geod(a=ellipsoid['semi_major_axis'],
        ...
        self.stations = self.stations[self.stations.within(
    ...
    def values(self) -> GeoDataFrame:
        ...
        values = values.iloc[:, [*range(5), *range(-len(self.variables), 0),
                                 *range(5, len(values.columns) -
                                        len(self.variables))]]
    ...
    def errors(self) -> GeoDataFrame:
        ...
        for _, station in self.stations.iterrows():
            station_modeled_values = values[values['station'] ==
                                            station['name']]
            ...
            for run_name in self.runs:
                run_modeled_values = station_modeled_values[[
                    'time', *(column for column in values.columns
                ...
                run_errors.columns = [f'{run_name}_{column}'
                                      if column in self.variables else column
    ...
    def rmses(self) -> {}:
        ...
        for _, station in self.stations.iterrows():
            ...
            station_rmses = []
            for run_name in self.runs:
                run_errors = station_errors[['stage', 'time',
                                             *(column
                ...
                run_rmses = DataFrame.from_dict(dict(zip(range(
                    len(run_rmses)), run_rmses)), orient='index')
                run_rmses.insert(0, 'run', run_name)
                station_rmses.append(run_rmses)

            station_rmses = pandas.concat(station_rmses)
            station_rmses = GeoDataFrame(station_rmses,
                                         geometry=[station.geometry for _ in
                                                   range(len(station_rmses))])
        ...
        for station_index, station in self.stations.iterrows():
            ...
            for variable_index, variable in enumerate(self.variables):
                axis = figure.add_subplot(
                    len(self.stations) * len(self.variables), 1,
    ...
    def plot_errors(self, show: bool = False):
        ...
        for variable_index, variable in enumerate(self.variables):
            axis = figure.add_subplot(len(self.variables), 1,
                                      variable_index + 1, sharex=sharing_axis)
            if sharing_axis is None:
                sharing_axis = axis
            ...
            for station_index, station in self.stations.iterrows():
                ...
                for run_index, run_name in enumerate(self.runs):
                    ...
                    for stage in self.stages:
                        ...
                        axis.plot(stage_errors['time'], stage_errors[variable],
                                  color=error_color,
                                  linestyle=LINESTYLES[stage])

        handles = [
            *(Line2D([0], [0], color=ERROR_COLOR_MAP(station_index_normalizer(
                station_index)), label=f'station {station["name"]}')
              for station_index, station in self.stations.iterrows()),
            Line2D([0], [0], color='k', linestyle=LINESTYLES['coldstart'],
                   label='coldstart'),
            Line2D([0], [0], color='k', linestyle=LINESTYLES['hotstart'],
        ...
        for variable, axis in error_axes.items():
            ...
            axis.hlines([0], *axis.get_xlim(), color='k', linestyle='--')
            axis.set_ylabel(f'{variable} error '
                            f'({ADCIRC_VARIABLES.loc[variable]["unit"]})')
            axis.legend(handles=handles)
    ...
    def plot_rmse(self, show: bool = False):
        ...
        for variable in self.variables:
            variable_rmses = rmses[
                ['run', 'station', 'stage', *(column
                                              for column in rmses.columns
                                              if column == variable)]]
            ...
            for stage in self.stages:
                ...
                for station_index, station in self.stations.iterrows():
                    station_rmses = stage_rmses[stage_rmses['station'] ==
                                                station['name']]
                    axis.plot(station_rmses['run'], station_rmses[variable],
